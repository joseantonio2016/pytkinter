...
class Application(ttk.Frame):
    ...
    def agregar_conjunto(self):
        ...
        btn_reemplazar.config(command=lambda archivo=entry_archivo, inicio=entry_linea_inicio, fin=entry_linea_fin: self.reemplazar_contenido(archivo.get(), inicio.get(), fin.get()) if all((archivo.get(), inicio.get(), fin.get())) else self.mostrar_mensaje())
        
        btn_agregar = ttk.Button(conjunto, text="Agregar")
        btn_agregar.pack(side='left', padx=5)
        btn_agregar.config(command=lambda archivo=entry_archivo, inicio=entry_linea_inicio, fin=entry_linea_fin: self.agregar_contenido(archivo.get(), inicio.get(), fin.get()) if all((archivo.get(), inicio.get(), fin.get())) else self.mostrar_mensaje())
